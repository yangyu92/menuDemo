...
- (void)dismissWithCompletion:(void (^)(JHCustomMenu *object))completion
{
    ...
    switch (_isAbout) {
        ...
        case AboutBarButtonItemRight: {
            [UIView animateWithDuration:0.2 animations:^{
                weakSelf.alpha = 0;
                weakSelf.tableView.frame = CGRectMake(weakSelf.origin.x + weakSelf.width, weakSelf.origin.y + TopToView, 0, 0);
            } completion:^(BOOL finished) {
                [weakSelf removeFromSuperview];
                if (completion) {
                    completion(weakSelf);
    ...
    switch (_isAbout) {
        case AboutBarButtonItemLeft: {
            ...
            CGContextMoveToPoint(context,
                                 LeftToView * 2.5+_origin.x, TopToView * 0.5+_origin.y);//设置起点
            
            CGContextAddLineToPoint(context,
                                    LeftToView * 2+_origin.x, TopToView+_origin.y);
            
            CGContextAddLineToPoint(context,
                                    LeftToView * 3+_origin.x, TopToView+_origin.y);
            break;
        }
        case AboutBarButtonItemRight:{
            
            CGContextMoveToPoint(context,
                                 LeftToView * 0.5+_origin.x + _width - LeftToView, TopToView * 0.5+_origin.y);//设置起点
            
            CGContextAddLineToPoint(context,
                                    _origin.x + _width - LeftToView, TopToView+_origin.y);
            
            CGContextAddLineToPoint(context,
                                    _origin.x + _width, TopToView+_origin.y);
            break;
        }
        default:
            break;
